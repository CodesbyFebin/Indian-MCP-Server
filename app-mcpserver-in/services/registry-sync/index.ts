...
import { RegistryEntry, RegistryIdentity, RegistrySource, RegistryVersion, RegistryChange, RegistryNormalizer, RegistryDeduplicator, RegistryValidator } from "@mcp/servers-registry";
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      try {
        ...
        const registryEntry: RegistryEntry = {
          id: parsed.id,
          type: (["server", "tool", "resource", "prompt"].includes(parsed.type)
            ? parsed.type
            : "server") as RegistryEntry["type"],
          name: parsed.name,
          ...
          creator: parsed.publisher ?? "unknown",
          createdAt: now,
          updatedAt: now,
        };
        const { identity, normalized: normalizedEntry } = normalizer.normalize(registryEntry, source);
